...
var panicSetup = function(serial, modNumber) {
	...
	this.placeCommand = function(modcode) {
		...
		if (commandTextElement[0].scrollHeight > 45) {
			var fontSize = 14;
			while (commandTextElement[0].scrollHeight > 45) {
				fontSize--;
				commandTextElement.css('font-size', fontSize+"px");
			}
		}
		//console.log(commandText, "GROUP: ", randPrefixGroup, "VALUE: ", randPrefix);
...
var wireSetup = function(serial, modNumber) {
	...
	this.place = function(wireArrays, modNumber) {
		...
		for (i = 0; i < 6; i++) {
			modcode.find('.indivWireCont').append('<button class="indivWires" id = "wire' + i + '"></button>');
...
(function initAll() {
	...
	console.log("CURRENT MODULES: ", spaces, "ANSWERS: ", answers);
}());
